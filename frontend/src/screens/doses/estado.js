...
const estado = ({navigation}) => {
    ...
    useEffect(() => {
      console.log("entrei no effect")
        ...
        .then((result) => {
          ...
          console.log(result.data[1].estado)
    ...
    useEffect(() => {
      ...
      let filtred = originalVacina.filter((d) => {
        return d.estado.includes(searchWorld)
      });
      setData(filtred)
    }, [searchWorld])
    ...
    renderItem = ({item}) => {
      return (
        <TouchableOpacity onPress={() => navigation.navigate("Doses", {doses: item.doses})}>
